<!-- ======= Hero Section ======= -->
  <section id="hero" class="d-flex align-items-center">

    <div class="container">
      <div class="row">
        <div class="col-lg-6 d-flex flex-column justify-content-center pt-4 pt-lg-0 order-2 order-lg-1" data-aos="fade-up" data-aos-delay="200">
          <h1>Sistema integrado del CEFA</h1>
          <h2><p>Centro de Formación Agroindustrial</p><p>"La Angostura"</p></h2>
          <div class="d-flex justify-content-center justify-content-lg-start">
            <!--video de prsentacion del sena--->
            <a href="#" class="btn-get-started scrollto glightbox"><i class="bi bi-play-circle"></i>&nbsp<span>Watch Video</span></a>
            
            <!--<a href="#" class="glightbox btn-watch-video"><i class="bi bi-play-circle"></i><span>Watch Video</span></a>-->
          </div>
        </div>
        <div class="col-lg-6 order-1 order-lg-2 hero-img" data-aos="zoom-in" data-aos-delay="200">
          <img src="<?php echo e(asset('general/assets/img/puzzle.png')); ?>" class="img-fluid animated zoom" alt="">
        </div>
      </div>
    </div>

  </section><!-- End Hero --><?php /**PATH C:\laragon\www\sicefados\resources\views/layouts/partials/hero.blade.php ENDPATH**/ ?>