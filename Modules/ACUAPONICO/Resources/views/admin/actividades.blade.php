                ...
                <table id="actividades" class="table table-hover table-bordered text-center align-middle">
                    ...
                    <tbody>
                        ...
                        <tr class="table-light">
                            ...
                            <td>
                                @if($activity->controls->isEmpty())
                                <button type="button" class="btn btn-primary btn-sm add-evidence-btn" data-toggle="modal" data-target="#agregar{{ $activity->id }}">
                                    <i class="fas fa-plus-circle mr-2"></i> + Evidencia
                                </button>
                                @else
                                <span class="text-success">Evidencia registrada</span>
                                @endif
                            </td>
                        </tr>
                        ...
                        <div class="modal fade" id="agregar{{ $activity->id }}" tabindex="-1" aria-labelledby="agregarLabel{{ $activity->id }}" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <form action="{{ route('acuaponico.pasante.pasante.storecontrolactivity') }}" method="post" enctype="multipart/form-data" id="form-agregar-{{ $activity->id }}">
                                    @csrf
                                    ...
                                    <div class="modal-content border-0 rounded-lg shadow-lg">
                                        ...
                                        <div class="modal-body p-5 bg-white rounded-bottom">
                                            ...
                                            <div class="form-group mb-4">
                                                ...
                                                <input type="file" name="evidence" class="form-control form-control-lg custom-input" accept="application/pdf" id="evidence-{{ $activity->id }}">
                                                <div class="invalid-feedback" id="error-date-{{ $activity->id }}" style="display:none;"></div>
                                            </div>
                                        </div>
                                        <div class="modal-footer bg-light p-4 rounded-bottom">
                                            ...
                                            <button type="submit" class="btn btn-primary btn-lg rounded-pill px-4" id="save-btn-{{ $activity->id }}">Guardar</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                    ...
                    <tbody>
                        ...
                        <tr class="table-light">
                            ...
                            <td>
                                ...
                                @endif
                                <button class="btn btn-sm btn-success edit-evidence-btn" data-toggle="modal" data-target="#editarModal{{ $evidencia->id }}">
                        ...
                        <div class="modal fade" id="editarModal{{ $evidencia->id }}" tabindex="-1" aria-labelledby="editarLabel{{ $evidencia->id }}" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <form action="{{ route('acuaponico.pasante.pasante.updatecontrolactivity', $evidencia->id) }}" method="POST" enctype="multipart/form-data" id="form-editar-{{ $evidencia->id }}">
                                    @csrf
                                    @method('PUT')
                                    <div class="modal-content border-0 rounded-lg shadow-lg">
                                        <div class="modal-header bg-gradient-primary text-white rounded-top p-4">
                                        ...
                                        <div class="modal-body p-5 bg-white rounded-bottom">
                                            <div class="form-group mb-4">
                                                ...
                                                <input type="date" name="date" class="form-control form-control-lg custom-input" value="{{ $evidencia->date }}" required>
                                            </div>
                                            <div class="form-group mb-4">
                                                <label class="form-label text-dark font-weight-bold custom-form-label">
                                                    <i class="fas fa-sticky-note mr-2"></i> Novedades
                                            ...
                                            <div class="form-group mb-4">
                                                ...
                                                <input type="file" name="evidence" class="form-control form-control-lg custom-input" accept="application/pdf" id="edit-evidence-{{ $evidencia->id }}">
                                                <div class="invalid-feedback" id="error-edit-date-{{ $evidencia->id }}" style="display:none;"></div>
                                            </div>
                                        </div>
                                        <div class="modal-footer bg-light p-4 rounded-bottom">
                                            <button type="button" class="btn btn-secondary btn-lg rounded-pill px-4" data-dismiss="modal">Cancelar</button>
                                            <button type="submit" class="btn btn-primary btn-lg rounded-pill px-4" id="update-btn-{{ $evidencia->id }}">Guardar Cambios</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        inputs.forEach(input => {
            input.value = today;
            input.readOnly = true; // Mantener readonly para evitar cambios manuales
        });

        // Validación de fecha y desactivación del botón de guardar (Agregar)
        @foreach($activities as $activity)
        const endDate = new Date("{{ $activity->end_date ?? '9999-12-31' }}");
        const form = document.getElementById('form-agregar-{{ $activity->id }}');
        const saveBtn = document.getElementById('save-btn-{{ $activity->id }}');
        const evidenceInput = document.getElementById('evidence-{{ $activity->id }}');
        const errorDiv = document.getElementById('error-date-{{ $activity->id }}');
        ...
        form.addEventListener('submit', (e) => {
            ...
            if (selectedDate > endDate) {
                ...
                errorDiv.textContent = 'No se puede registrar evidencia después de la fecha de finalización de la actividad.';
                saveBtn.disabled = true;
            } else if (!evidenceInput.files.length) {
                e.preventDefault();
                errorDiv.style.display = 'block';
                errorDiv.textContent = 'Debe subir un archivo PDF como evidencia.';
                saveBtn.disabled = true;
            } else {
        ...
        evidenceInput.addEventListener('change', () => {
            if (evidenceInput.files.length && new Date(today) <= endDate) {
                errorDiv.style.display = 'none';
                saveBtn.disabled = false;
            }
        });
        ...
        // Validación de fecha y desactivación del botón de guardar (Editar)
        @foreach($evidencias as $evidencia)
        const endDateEdit = new Date("{{ $evidencia->activity->end_date ?? '9999-12-31' }}");
        const editForm = document.getElementById('form-editar-{{ $evidencia->id }}');
        const updateBtn = document.getElementById('update-btn-{{ $evidencia->id }}');
        const editEvidenceInput = document.getElementById('edit-evidence-{{ $evidencia->id }}');
        const editErrorDiv = document.getElementById('error-edit-date-{{ $evidencia->id }}');

        editForm.addEventListener('submit', (e) => {
            const selectedDate = new Date(document.querySelector('#form-editar-{{ $evidencia->id }} [name="date"]').value);
            if (selectedDate > endDateEdit) {
                e.preventDefault();
                editErrorDiv.style.display = 'block';
                editErrorDiv.textContent = 'No se puede actualizar la evidencia después de la fecha de finalización de la actividad.';
        ...
        editEvidenceInput.addEventListener('change', () => {
            if (new Date(document.querySelector('#form-editar-{{ $evidencia->id }} [name="date"]').value) <= endDateEdit) {
                editErrorDiv.style.display = 'none';
                updateBtn.disabled = false;
            }
        });
